...
LRESULT WINAPI SnapHelperWinProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
        {
            SIZEMOVEDATA *sizemove = (SIZEMOVEDATA*)LSMGet();
            HDC hDC = GetDC(hwnd);
...
BOOL CALLBACK EnumWindowsShakeMinimize(HWND hwnd, LPARAM lParam)
{
    ...
    if(hwnd != sizemove->hwnd && IsWindowVisible(hwnd) && !IsIconic(hwnd))
    {
        ...
        */
        ...
        pNode = (SHAKERESTORENODE*)LocalAlloc(0, sizeof(SHAKERESTORENODE));
        if(!pNode)
            return FALSE;
...
void LSMCleanup()
{
    SIZEMOVEDATA *sm = (SIZEMOVEDATA*)LSMGet();
    if(sm)
    {
